...
class AbstractCar:
    ...
    def get_rotated_img(self):
        ...
        rotated_rect = rotated_image.get_rect(center = (self.x + self.img.get_width() / 2, self.y + self.img.get_height() / 2))
        
        return rotated_image, rotated_rect

    def collide(self, mask, x=0, y=0):
        rImage, rRect = self.get_rotated_img()
        carMask = pygame.mask.from_surface(rImage)
        # WIN.blit(mask_image, rRect) # Uncomment to see the mask on the screen
        pygame.display.update()
        offset = (int(rRect.x - x), int(rRect.y - y))
...
def handle_collision(player_car, computer_car, game_info):
    if player_car.collide(TRACK_BORDER_MASK, *(TRACK_BORDER_RECT.x,TRACK_BORDER_RECT.y)) != None:
        player_car.bounce()
    # else: # Uncomment to see the car change color when it collides with the border
    #     player_car.img = RED_CAR


    # i would commit this but it would break the computer car and i can be bothered to update the path 
    # if computer_car.collide(TRACK_BORDER_MASK) != None: 
    #     computer_car.bounce()
    # else:
    #     computer_car.img = GREEN_CAR
