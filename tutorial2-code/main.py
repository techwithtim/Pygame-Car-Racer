...
class AbstractCar:
    ...
    def sense(self, win):
        (x1, y1), (x2, y2) = TRACK_LINES[0]
        (x3, y3), (x4, y4) = self.sensors[0]
        x1 = round(x1)
        x2 = round(x2)
        x3 = round(x3)
        x4 = round(x4)
        ...
        y3 = round(y3)
        y4 = round(y4)
        print(TRACK_LINES[0], self.sensors[0])
        if x1 == x2 and x3 == x4:
            pass
        elif x1 == x2:
            print("im here bioa")
            ...
            if min(y1,y2) <= y <= max(y1, y2) and min(x3,x4) <= x1 <= max(x3,x4):
            ...
            if min(y3,y4) <= y <= max(y3, y4):
                pygame.draw.circle(WIN, (255,255,255),(x3, y), 5)
        else:
            pass
